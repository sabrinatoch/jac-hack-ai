...
__export(agents_exports, {
  HfAgent: () => HfAgent,
  LLMFromEndpoint: () => LLMFromEndpoint,
  LLMFromHub: () => LLMFromHub,
  defaultTools: () => defaultTools,
  imageToTextTool: () => imageToTextTool,
...
async function evalBuilder(code, tools, files, updateCallback, accessToken) {
  async function wrapperEval() {
    ...
    for (const tool of tools) {
      const toolCall = (input) => tool.call?.(input, new import_inference.HfInference(accessToken ?? ""));
...
function LLMFromHub(accessToken, model) {
  const inference = new import_inference2.HfInference(accessToken);
  return async (prompt) => {
...
function LLMFromEndpoint(acessToken, endpoint) {
  const inference = new import_inference2.HfInference(acessToken).endpoint(endpoint);
  return async (prompt) => {
    const formattedPrompt = "<|user|>" + prompt + "<|end|><|assistant|>";
    const output = await inference.textGeneration({
      inputs: formattedPrompt,
