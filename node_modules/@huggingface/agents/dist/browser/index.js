  ...
  async function wrapperEval() {
    ...
    for (const tool of tools) {
      const toolCall = (input) => {
        var _a;
        return (_a = tool.call) == null ? void 0 : _a.call(tool, input, new (0, _inference.HfInference)(accessToken != null ? accessToken : ""));
      };
...
async function generateCode(prompt, tools, files, llm) {
  ...
  try {
    ...
    return (_a = codeBlocks[0].replace("js", "").replace("javascript", "").trim()) != null ? _a : "nothing";
  } catch (e) {
...
function LLMFromHub(accessToken, model) {
  const inference = new (0, _inference.HfInference)(accessToken);
  return async (prompt) => {
    const formattedPrompt = "<|user|>" + prompt + "<|end|><|assistant|>";
    const output = await inference.textGeneration({
      inputs: formattedPrompt,
      model: model != null ? model : "OpenAssistant/oasst-sft-4-pythia-12b-epoch-3.5",
      parameters: {
        max_new_tokens: 900
      }
    });
...
function LLMFromEndpoint(acessToken, endpoint) {
  const inference = new (0, _inference.HfInference)(acessToken).endpoint(endpoint);
  return async (prompt) => {
    const formattedPrompt = "<|user|>" + prompt + "<|end|><|assistant|>";
    const output = await inference.textGeneration({
      inputs: formattedPrompt,
...
var HfAgent = class {
  constructor(accessToken = "", LLM, tools) {
    ...
    this.llm = LLM != null ? LLM : LLMFromHub(accessToken);
    this.tools = tools != null ? tools : defaultTools;
  }
...
exports.HfAgent = HfAgent; exports.LLMFromEndpoint = LLMFromEndpoint; exports.LLMFromHub = LLMFromHub; exports.defaultTools = defaultTools; exports.imageToTextTool = imageToTextTool; exports.speechToTextTool = speechToTextTool; exports.textToImageTool = textToImageTool; exports.textToSpeechTool = textToSpeechTool;
